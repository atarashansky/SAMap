...
def compute_csim(sam3,key,X=None):
    ...
    cl = q(sam3.adata.obs['batch'].values.astype('object')+'_'+sam3.adata.obs[key].values.astype('str').astype('object'))
...
def get_mapping_scores(sam1,sam2,samap,key1,key2):
    ...
    cl = q(samap.adata.obs['species']).astype('object') +'_'+ np.append(cl1,cl2).astype('str').astype('object')
